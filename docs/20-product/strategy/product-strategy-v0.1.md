...
> **Status: SUPERSEDED.** 현재 전략은 `product-baseline-v2.md`와
> `platform-strategy-v2-research-v0.1.md`를 따른다.

## 1. Executive Summary: The Sharp Angle
본 제품은 **"음악을 재생하는 도구"**가 아니라, **"플랫폼에 갇힌 인간의 취향을 해방하고 자산화하는 레이어"**다. 

우리는 Spotify나 Apple Music과 경쟁하지 않는다. 대신, 그들 사이를 흐르는 **'맥락이 담긴 데이터(Contextual Data)'**의 주권을 사용자에게 돌려줌으로써, 플랫폼 밖에서도 살아남는 취향 자산의 저장소가 된다.
...
3.  **기록의 비영속성 (Memory Fragility):** 사용자가 공들여 만든 취향 기록은 플랫폼을 떠나는 순간 보존과 재사용이 어려워진다.
...
- **Moat (방어기제):** 플랫폼이 쉽게 복제하기 어려운 '감정 맥락'과 그들 사이를 잇는 '정규화된 원장(Ledger)'.
...
| **Future B2B Data Sales** | **Future Expansion** | **MVP 배제.** 데이터 스키마 제약으로만 고려하고, 기획/마케팅/사용자 가치 제안에서는 완전히 삭제한다. |

---

## 5. Value Proposition Hierarchy (가치 제안 계층)
1.  **[Bottom - Utility]:** 플랫폼 간 플레이리스트 이동 (Spotify -> Apple Music).
2.  **[Middle - Emotional]:** 내 음악 취향에 일기, 사진, 감정을 담아 기록 (자산화).
3.  **[Top - Social/Comm.]:** 소중한 사람에게 내 취향 자산을 공유 (연결).

---

## 6. MVP Boundary & Priority Framework (P0/P1/P2)

모든 것을 중요하다고 하지 않는다. **'Asymmetric Emotional Portability'** 루프 완성에만 집중한다.

### P0: 핵심 루프 (Must have for Launch)
- **Import:** Spotify 플레이리스트 URL을 넣으면 곡 목록을 긁어옴.
- **Normalize:** ISRC 기반으로 곡을 식별하여 우리만의 'Ledger'에 저장.
- **Personalize:** 커버 이미지, 한 줄 일기, 감정 태그를 붙여 'PA'로 전환.
- **Save & Share:** 로그인 없이도 볼 수 있는 아름다운 '웹 감상 페이지' 링크 생성.
- **Export:** 저장된 PA를 Apple Music 플레이리스트로 내보내기.

### P1: 가치 강화 (Should have shortly after)
- **Bidirectional Bridge:** Apple Music 가져오기 및 Spotify로 내보내기 지원.
- **Humanity Proof (Basic):** 작성 시간, 편집 패턴 등 기초적인 인간 생성 신호 수집 및 표시.
...
3.  **Data Privacy:** 일기, 사진 등 민감 정보 수집에 대한 거부감.
    - *Mitigation:* 민감 정보 수집을 최소화하고, 초기에는 저장 보호와 선택적 공개 제어를 강조.
...
- **"센서 기반 proof를 P0에 넣을까?"** -> 아니오. P0는 정서적 맥락에 집중하고, 기술적 proof signal은 P1로 미룬다.
- **"B2B를 지금 넣을까?"** -> 아니오. future expansion으로만 둔다.
